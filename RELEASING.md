...
1. Update `package.json` and `package-lock.json`, the version lines in `README.md`/`README.ko.md`, and `APP_VERSION` in `src/app/game/GamePageClient.tsx`. Remove the temporary npm `2026.9.18`/`2026.9.19` warning from both READMEs after validating the fixed tarball. The tag must equal the package version. Also bump every **pinned example image tag** so the docs never point at a release older than the one being cut — `deploy/hostinger/docker-compose.yml` (`DESKRPG_IMAGE` default), the `DESKRPG_IMAGE=` pin examples in both READMEs, and the `OFFICE_IMAGE` pin example in `deploy/office/README.md`. Grep the previous version string to catch stragglers; leave historical minimum-version notes (for example the npm `2026.9.20`-or-later warning) alone.
