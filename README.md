...
- Version: `v2026.921.4` — The whole loop now runs end to end: after a meeting you register the follow-up tasks (or decline), the screen returns to the office by itself a few seconds later, approved cards start right away, and staff walk over to report in the order their work finished — the chat opens with a summary of what they are reporting, sending them back marks that report as read, and the report badge opens a list of what is still waiting (with **Call again** for reports you closed). Staff keep moving when the tab is in the background, reports no longer stall the queue, files produced by card runs are collected under Results per card (needs plugin 0.12.1 — update it from the gateway screen), and the meeting camera closes in on the speaker. The meeting screen's exit moved from the top bar to an **Office** button on the map, the web UI adopts the Dante Labs brand colors with readable contrast, and there is a GitHub Star button, a new-version dot, an optional experience survey and a bug report window — see _What is sent_ below for exactly what leaves your server and how to turn it off. Earlier in 2026.921.3: structured meeting outcomes, approvals, card proposals, the Cards tab and the run timeline.
