...
type GltfDocument = {
  ...
  accessors: { count: number; min?: number[]; max?: number[]; bufferView: number; byteOffset?: number; componentType: number; type: string }[];
...
async function main() {
  ...
  for (const look of OFFICE_LOOKS) {
    ...
    assert.ok(gltf.buffers.every((b) => !b.uri), `${path}: GLB must be self-contained`);
    assert.ok((gltf.images ?? []).every((i) => !i.uri || i.uri.startsWith("data:")), path);
    ...
    for (const clip of gltf.animations) {
      ...
      for (const sampler of clip.samplers) {
        ...
        assert.ok(time.count >= 2 && time.max?.[0] && time.max[0] > 0, `${path}: ${clip.name} duration`);
      }
      ...
      for (const channel of clip.channels.filter((c) => c.target.path === "scale")) {
        ...
        assert.notEqual(sampler.interpolation, "CUBICSPLINE", `${path}: review scale spline explicitly`);
        ...
        for (let frame = 0; frame < accessor.count; frame++) {
          const value = [0, 1, 2].map((axis) => bytes.readFloatLE(start + frame * (view.byteStride ?? 12) + axis * 4));
          const reference = scaleByNode.get(channel.target.node) ?? value;
          assert.ok(value.every((v, axis) => Number.isFinite(v) && Math.abs(v - reference[axis]) < 0.001), `${path}: ${gltf.nodes?.[channel.target.node]?.name} changes scale in ${clip.name}`);
    ...
    for (const mesh of gltf.meshes) for (const primitive of mesh.primitives) {
      ...
      assert.ok(positions.count > 0 && positions.min?.every(Number.isFinite) && positions.max?.every(Number.isFinite), path);
    ...
    results.push({
      id: look.id, name: look.name, bodyType: look.bodyType, outfit: look.outfit,
      bytes: bytes.length, triangles, nodes: gltf.nodes?.length,
      sha256: createHash("sha256").update(bytes).digest("hex"),
      errors: validation?.issues.numErrors ?? null,
      warnings: validation?.issues.numWarnings ?? null,
      warningCodes: validation ? [...new Set(validation.issues.messages.filter((m: { severity: number }) => m.severity === 1).map((m: { code: string }) => m.code))] : [],
    });
  }
  assert.equal(new Set(results.map((r) => r.sha256)).size, OFFICE_LOOKS.length, "Duplicate model files");
  const report = { generatedAt: new Date().toISOString(), count: results.length, totalBytes: results.reduce((s, r) => s + r.bytes, 0), validator: !!validator, results };
  mkdirSync("art/characters/office-catalog", { recursive: true });
  writeFileSync("art/characters/office-catalog/validation.json", JSON.stringify(report, null, 2) + "\n");
  console.log(JSON.stringify({ count: report.count, totalBytes: report.totalBytes, validator: report.validator, errors: results.reduce((s, r) => s + (r.errors ?? 0), 0) }));
}
