...
- 버전: `v2026.921.1` — Windows 에서 네이티브로 돕니다. 연결 마법사가 로컬 Hermes 를 찾거나 설치하고 SSH 로 리눅스 호스트에 붙습니다. 대화창은 아바타로 발화자를 구분하고, 직원과의 DM 이 대화 목록에 올라오며, 한 줄 링크는 미리보기 카드가 되고, 대화 속 파일·이미지를 내려받을 수 있습니다. 직원이 대표석에 앉지 않고, 칸반 카드를 누르면 상세가 다시 열리며, 카드 첨부가 결과물과 함께 나열됩니다. 앞선 2026.920.x: 이메일 없는 비밀번호 복구, python3 없는 서버에서도 되는 로컬·SSH 호스트 연결, 3D 미니어처 로고.
