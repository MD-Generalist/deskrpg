...
- 버전: `v2026.921.2` — 칸반 카드의 실행 기록 시각이 1970년으로 찍히고 경과 시간이 보이지 않던 문제를 고쳤습니다. 같은 보드를 목록으로도 볼 수 있고(서브프로젝트·담당·상태로 묶기, 필터, 정렬, 진행률, 하위 업무 펼치기), 카드가 검토·차단·완료가 되면 담당 직원이 걸어와 보고합니다. 직원 호출과 회의 뒤 복귀가 안정됐고, 연결된 플러그인의 버전을 화면에서 확인하고 갱신할 수 있습니다. 앞선 2026.921.1: Windows 네이티브 지원, 대화창 아바타, 직원 DM 목록, 링크 미리보기, 파일 내려받기.
