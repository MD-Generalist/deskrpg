...
- 버전: `v2026.921.4` — 업무 흐름이 처음부터 끝까지 이어집니다. 회의가 끝나 후속 업무를 등록하면(또는 등록하지 않으면) 몇 초 뒤 화면이 스스로 오피스로 돌아가고, 승인한 카드는 곧바로 시작되며, 직원은 일이 끝난 순서대로 걸어와 보고합니다 — 대화창 맨 위에 무엇을 보고하는지 요약이 뜨고, 직원을 복귀시키면 그 보고가 확인되며, 보고 배지는 남은 보고 목록을 엽니다(닫아 둔 보고는 **다시 부르기**). 탭이 뒤에 있어도 직원이 계속 움직이고, 보고가 큐를 멈추지 않으며, 카드 실행으로 만든 파일이 카드별로 결과물에 쌓이고(플러그인 0.12.1 필요 — 게이트웨이 화면에서 갱신하세요), 회의 카메라가 발언자를 가까이 잡습니다. 회의 화면의 출구는 상단 바에서 맵 위 **오피스로** 버튼으로 옮겼고, 웹 화면은 읽기 좋은 대비의 단테랩스 브랜드 색을 쓰며, GitHub Star 버튼·새 버전 점·선택형 사용 경험 설문·버그 리포트 창이 생겼습니다 — 무엇이 서버 밖으로 나가고 어떻게 끄는지는 아래 _보내는 데이터_ 절에 있습니다. 앞선 2026.921.3: 구조화된 회의 결과, 승인, 카드 제안, 카드 탭, 실행 타임라인.
