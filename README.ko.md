...
- 버전: `v2026.920.2` — 연결 마법사가 로컬·SSH 호스트에 붙고, python3 가 없는 서버에도 Hermes 를 설치합니다. 화면의 안내를 줄였습니다 — 막힌 이유는 `?` 뒤로, 직원 등록은 이전·다음으로, 오피스 참여 코드는 팝업으로 옮겼습니다. 3D 미니어처 로고와 공통 본문 틀을 적용했습니다.
