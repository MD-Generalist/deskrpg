...
- 버전: `v2026.920.5` — 이메일 없이 비밀번호를 복구합니다. 관리자가 재설정하고, 관리자 본인이 잠기면 호스트에서 CLI 로 풀고, 본인은 계정 화면에서 바꿉니다. 로그인 화면도 앱과 같은 브랜드로 맞췄습니다. 앞선 2026.920.x: 로컬·SSH 호스트 연결과 python3 없는 서버 설치, 화면 안내 축소, 3D 미니어처 로고.
