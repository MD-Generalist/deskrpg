    ...
    >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 py-3">
        <section aria-labelledby="workspace-spaces-heading">
          <h3 id="workspace-spaces-heading" className="px-2 pb-1 text-[11px] font-bold text-text-dim">
            {t("workspace.spaces")}
          </h3>
            ...
            {sortRooms(props.rooms).map((room) => (
              <button
                ...
                className={`w-full rounded-lg px-3 py-2 text-left ${
                  room.id === props.currentRoomId ? "bg-surface-raised" : "hover:bg-surface-raised/70"
                }`}
              >
        ...
        <section className="mt-5" aria-labelledby="workspace-people-heading">
          <h3 id="workspace-people-heading" className="px-2 pb-1 text-[11px] font-bold text-text-dim">
            {t("workspace.people")}
          </h3>
      ...
      {selectedNpc && (
        <div role="menu" className="m-2 rounded-lg border border-border bg-surface p-1 shadow-xl">
          {selectedNpc.motion === "idle" && selectedNpc.active && selectedNpc.placed && (
            <button role="menuitem" onClick={() => action("call")} className="w-full rounded px-3 py-2 text-left text-sm hover:bg-surface-raised">
              {t("workspace.action.call")}
            </button>
          )}
          {selectedNpc.motion === "waiting" && selectedNpc.calledByViewer && (
            <button role="menuitem" onClick={() => action("return")} className="w-full rounded px-3 py-2 text-left text-sm hover:bg-surface-raised">
              {t("npc.return")}
            </button>
          )}
          {props.isOwner && selectedNpc.active && !selectedNpc.placed && (
            <button role="menuitem" onClick={() => action("place")} className="w-full rounded px-3 py-2 text-left text-sm hover:bg-surface-raised">
          ...
          {props.isOwner && (
            <>
              ...
              <button role="menuitem" onClick={() => action("profile")} className="w-full rounded px-3 py-2 text-left text-sm hover:bg-surface-raised">
                {t("workspace.action.profile")}
              </button>
            </>
          )}
          <button role="menuitem" onClick={() => action("reset-chat")} className="w-full rounded px-3 py-2 text-left text-sm hover:bg-surface-raised">
            {t("context.resetChat")}
          </button>
          {props.isOwner && (
            <button
