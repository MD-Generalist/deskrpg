...
export type MeetingExecutionState = Pick<MeetingDiscussionState,
...
export function restoreMeetingExecution(state: MeetingExecutionState | null | undefined) {
  ...
  return {
    rawStreams,
    streams: Object.fromEntries(Object.entries(rawStreams).map(([id, body]) =>
      [id, sanitizeClientStreamingSpeech(body)])),
...
export function restoreMeetingChat<T extends { senderType: string; content: string }>(
  ...
  return {
    ...
    messages: discussion ? messages.map((message) => ({
      ...message,
      content: message.senderType === "npc"
        ? sanitizeClientFinalSpeech(message.content) : message.content,
    })) : [],
