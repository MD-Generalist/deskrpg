...
interface StampEditorModalProps {
  ...
  onOpenPixelEditor: (
    ...
    tileWidth: number,
    tileHeight: number,
    onResult: (dataUrl: string, newCols: number, newRows: number) => void,
  ) => void;
...
export default function StampEditorModal({
  ...
  const [layers, setLayers] = useState<StampLayerData[]>(
    parseLayers(stamp.layers, stamp.cols, stamp.rows, mapLayerNames),
  );
  ...
  const getTileOwnerLayer = useCallback(
  ...
  const renderCanvas = useCallback(() => {
    ...
    for (let x = 0; x <= stampCols; x++) {
      ctx.beginPath();
      ctx.moveTo(x * ds, 0);
      ctx.lineTo(x * ds, stampRows * ds);
      ctx.stroke();
    }
    for (let y = 0; y <= stampRows; y++) {
      ctx.beginPath();
      ctx.moveTo(0, y * ds);
      ctx.lineTo(stampCols * ds, y * ds);
      ctx.stroke();
    }
  ...
  const moveTileToLayer = useCallback(
  ...
  const handleEditPixels = useCallback(() => {
    ...
    onOpenPixelEditor(
      ...
      stamp.tileWidth,
      stamp.tileHeight,
      (resultDataUrl: string, newCols: number, newRows: number) => {
        ...
        if (existingTs) {
          for (let li = 0; li < currentLayers.length; li++) {
            ...
            for (const gid of currentLayers[li].data) {
              if (
                gid !== 0 &&
                gid >= existingTs.firstgid &&
                gid < existingTs.firstgid + existingTs.tilecount
              ) {
                isShared = true;
                break;
              }
  ...
  const selectedTileOwner = selectedTile
    ? getTileOwnerLayer(
        Math.min(selectedTile.col, selectedTile.col2),
        Math.min(selectedTile.row, selectedTile.row2),
      )
    : -1;
    ...
    >
      <div className="flex" style={{ height: "60vh" }}>
        ...
        <div className="w-48 border-r border-border p-2 flex flex-col gap-1 flex-shrink-0 overflow-y-auto">
          <div className="text-micro text-text-dim uppercase tracking-wider mb-1">
            {t("mapEditor.stamps.layers")}
          </div>
          {layers.map((layer, idx) => {
            ...
            return (
              <button
                ...
                style={
                  isActive ? { backgroundColor: `${lc.solid}15`, borderColor: `${lc.solid}40` } : {}
                }
              >
                <div
                ...
                />
                <span
                  className={`text-caption truncate ${isActive ? "text-text" : "text-text-secondary"}`}
                >
                  {layer.name}
                </span>
              ...
              <div className="mt-1">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setLayers((prev) => [
                      ...prev,
                      { name, type: "tilelayer", depth: 0, data: emptyData },
                    ]);
                    setDirty(true);
                  }}
                >
              ...
              return (
                <div className="mt-2 pt-2 border-t border-border">
                  <div className="text-micro text-text-dim mb-1.5">
                    {t("mapEditor.stamps.selectedTile")}{" "}
                    {isSingle
                      ? `(${minCol}, ${minRow})`
                        ...
                        return (
                          <div key={li} className="flex items-center gap-1.5">
                            <div
                              ...
                              style={{ backgroundColor: lc.solid }}
                            />
                            <span className="text-micro text-text-secondary flex-1 truncate">
                              {layer.name}
                            </span>
