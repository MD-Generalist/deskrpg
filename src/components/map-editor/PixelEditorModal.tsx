  ...
  if (editing) {
    return (
      <input
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter") {
            onChange(clamp(Number((e.target as HTMLInputElement).value) || min));
            setEditing(false);
          }
...
export default function PixelEditorModal({
  ...
  const regionPxW = isDirectImage
    ...
    : region && tilesetInfo
      ? region.width * tilesetInfo.tilewidth
      : 0;
  const regionPxH = isDirectImage
    ? (initialRows ?? 1) * (initialTileHeight ?? 32)
    : region && tilesetInfo
      ? region.height * tilesetInfo.tileheight
      : 0;
  ...
  const buildCheckerboard = useCallback((pixelW: number, pixelH: number, z: number) => {
  ...
  const hitTestHandle = useCallback(
    (e: React.MouseEvent): HandleType | null => {
  ...
  const renderCanvas = useCallback(() => {
    ...
    if (
      tool === "magic-eraser" &&
      magicEraserPreviewRef.current &&
      magicEraserPreviewRef.current.size > 0
    ) {
    ...
    if (ps) {
      if (isPixelPasteMode) {
        ...
        const selHandles = getHandlePositions(
          { x: ps.x, y: ps.y, width: ps.width, height: ps.height } as TransformState,
          zoom,
        );
    ...
    if (isPixelPasteMode && pixelClipboard && hp) {
      ...
      ctx.drawImage(
        ...
        hp.x * zoom,
        hp.y * zoom,
        pixelClipboard.width * zoom,
        pixelClipboard.height * zoom,
      );
      ctx.globalAlpha = 1;
      ctx.strokeStyle = "rgba(59, 130, 246, 0.9)";
      ctx.lineWidth = 1;
  ...
  useEffect(() => {
    const needsAnimation =
      isPixelPasteMode ||
      transformActive ||
      (tool === "rect-select" && pixelSelection && !isRectSelectingRef.current);
  ...
  const computeMagicEraserPreview = useCallback(
    (px: number, py: number) => {
      const ec = editCanvasRef.current;
      if (!ec) return;
      ...
      if (tool === "rect-select") {
        ...
        if (pixelSelection && pixelSelection.width >= 1 && pixelSelection.height >= 1) {
          ...
          const handles = getHandlePositions(
            { x: sel.x, y: sel.y, width: sel.width, height: sel.height } as TransformState,
            zoom,
          );
          const hh = HANDLE_HIT / 2;
          ...
          for (const [key, p] of Object.entries(handles) as [
            HandleType,
            { x: number; y: number },
          ][]) {
            if (
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        const idx = currentIdx === -1 ? ZOOM_LEVELS.findIndex((z) => z >= oldZoom) : currentIdx;
      ...
      if (mod && e.shiftKey && (e.key === "z" || e.key === "Z")) {
        ...
        if (ec && pixelSelection) {
          ...
          const id = ctx.getImageData(
            pixelSelection.x,
            pixelSelection.y,
            pixelSelection.width,
            pixelSelection.height,
          );
        ...
        if (e.key === "Escape") {
          ...
          e.preventDefault();
          setBrushSize((s) => Math.min(16, s + 1));
        } else if (e.key === "?") {
          e.preventDefault();
          setShowHelp((v) => !v);
        }
  ...
  const handleSaveAsNew = useCallback(() => {
    ...
    const maxCols =
      tileCount === 1
        ? origColumns
        : Math.min(expandedCols, Math.max(origColumns, Math.ceil(Math.sqrt(tileCount))));
  ...
  // --- Cursor style ---
  const hasSelectionHandles =
    !transformActive && tool === "rect-select" && pixelSelection && transformCursor !== "default";
        ...
        <div className="flex items-center h-10 bg-surface border-b border-border px-1 select-none flex-shrink-0">
          ...
          <div className="flex items-center gap-1 px-2 border-r border-border">
            <Tooltip label={t("mapEditor.pixel.penTooltip")} shortcut="P">
              <Button
                variant={!tileEditMode && tool === "pen" ? "primary" : "ghost"}
                size="sm"
            ...
            <Tooltip label={t("mapEditor.pixel.eraserTooltip")} shortcut="E">
              <Button
                variant={!tileEditMode && tool === "eraser" ? "primary" : "ghost"}
                size="sm"
            ...
            <Tooltip label={t("mapEditor.pixel.eyedropperTooltip")} shortcut="I">
              <Button
                variant={!tileEditMode && tool === "eyedropper" ? "primary" : "ghost"}
                size="sm"
            ...
            <Tooltip label={t("mapEditor.pixel.shiftTooltip")} shortcut="V">
              <Button
                variant={!tileEditMode && tool === "shift" ? "primary" : "ghost"}
                size="sm"
            ...
            <Tooltip label={t("mapEditor.pixel.selectTooltip")} shortcut="M">
              <Button
                variant={!tileEditMode && tool === "rect-select" ? "primary" : "ghost"}
                size="sm"
                onClick={() => switchTool("rect-select")}
              >
          ...
          {tool === "magic-eraser" && (
            <div className="flex items-center gap-1.5 px-2 border-l border-border">
              <span className="text-micro text-secondary whitespace-nowrap">
                {t("mapEditor.pixel.tolerance")}
              </span>
              <DialInput
                value={magicEraserTolerance}
                onChange={(v) => {
                  setMagicEraserTolerance(v);
                  magicEraserCacheKeyRef.current = "";
                }}
                min={0}
                max={100}
                label={t("mapEditor.pixel.tolerance")}
              />
            </div>
          )}
          ...
          <div className="flex items-center gap-0.5 px-2 border-r border-border">
            <Tooltip label={t("mapEditor.pixel.magicEraser")} shortcut="G">
              <Button
                variant={!tileEditMode && tool === "magic-eraser" ? "primary" : "ghost"}
                size="sm"
                onClick={() => switchTool("magic-eraser")}
              >
            ...
            <Tooltip label={t("mapEditor.pixel.tileEditMode")}>
              <Button
                variant={tileEditMode ? "primary" : "ghost"}
                size="sm"
                onClick={() => {
                  setTileEditMode((v) => !v);
                  setHoveredEdge(null);
        ...
        >
          ...
          {hoveredEdge &&
            (() => {
              ...
              const delCls =
                "w-full h-full bg-red-500/20 hover:bg-red-500/35 border border-red-500/40 hover:border-red-500/60 transition-colors";
              const addCls =
                "w-full h-full bg-green-500/15 hover:bg-green-500/30 border border-dashed border-green-500/30 hover:border-green-500/50 transition-colors";
              ...
              if (hoveredEdge.top === "delete") {
                zones.push(
                  <div
                    ...
                    title={t("mapEditor.pixel.deleteTopRowTooltip")}
                ...
                zones.push(
                  <div
                    key="add-top"
                    className="absolute cursor-pointer"
                    title={t("mapEditor.pixel.addTopRowTooltip")}
              ...
              if (hoveredEdge.bottom === "delete") {
                zones.push(
                  <div
                    key="del-bottom"
                    className="absolute cursor-pointer"
                    title={t("mapEditor.pixel.deleteBottomRowTooltip")}
                ...
                zones.push(
                  <div
                    key="add-bottom"
                    className="absolute cursor-pointer"
                    title={t("mapEditor.pixel.addBottomRowTooltip")}
              ...
              if (hoveredEdge.left === "delete") {
                zones.push(
                  <div
                    key="del-left"
                    className="absolute cursor-pointer"
                    title={t("mapEditor.pixel.deleteLeftColTooltip")}
                ...
                zones.push(
                  <div
                    key="add-left"
                    className="absolute cursor-pointer"
                    title={t("mapEditor.pixel.addLeftColTooltip")}
              ...
              if (hoveredEdge.right === "delete") {
                zones.push(
                  <div
                    key="del-right"
                    className="absolute cursor-pointer"
                    title={t("mapEditor.pixel.deleteRightColTooltip")}
                ...
                zones.push(
                  <div
                    key="add-right"
                    className="absolute cursor-pointer"
                    title={t("mapEditor.pixel.addRightColTooltip")}
              ...
              return (
                <div className="absolute inset-0 pointer-events-none" style={{ zIndex: 10 }}>
                  <div className="pointer-events-auto">{zones}</div>
                </div>
              );
            })()}
        </div>
        ...
        {onApply ? (
          ...
          <>
            ...
            <Button
              ...
              title={
                !isDirectImage && isExpanded ? t("mapEditor.pixel.overwriteDisabled") : undefined
              }
            >
