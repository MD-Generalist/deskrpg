...
const valid = { ambientSchedule: { phase: "roam", elapsed: 1000, duration: 25000, pause: 600 }, ambientTimer: 200, ambientSeat: { x: 1, y: 1 }, path: [{ x: 4, y: 4 }] };
...
test("motion continuation rejects malformed phases, unbounded payloads and off-map coordinates", () => {
  for (const patch of [
    ...
    { ambientTimer: NaN }, { ambientSeat: { x: -1, y: 1 } },
    { path: Array.from({ length: 257 }, () => ({ x: 4, y: 4 })) },
    { path: [{ x: 4, y: 16 }] }, { path: [null] },
  ]) assert.equal(parseMotionContinuation({ ...valid, ...patch }, { width: 512, height: 512 }).ok, false, JSON.stringify(patch));
});
...
test("motion continuation copies only validated data and distinguishes omission from explicit reset", () => {
  ...
  const result = parseMotionContinuation({ ...valid, extra: "not part of protocol" }, { width: 512, height: 512 });
  assert.ok(result.ok);
