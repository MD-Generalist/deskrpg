...
test("channel tasks resolve assigned profile display names and preserve backlog through rename/fallback", async () => {
  ...
  const [character] = await db.insert(characters).values({ userId: seed.userId, name: "Tester", appearance: "{}" }).returning();
  const manager = new TaskManager(db, { tasks, npcs, hermesProfiles });
  const backlog = await manager.createBacklogTask(seed.channelId, character.id, "Original backlog", null);
  ...
  await db.update(hermesProfiles).set({ displayName: "Renamed profile" }).where(eq(hermesProfiles.id, npc.hermesProfileId));
  list = await manager.getTasksByChannel(seed.channelId);
  assert.equal(list[0].npcName, "Renamed profile");
  await db.update(hermesProfiles).set({ displayName: "  " }).where(eq(hermesProfiles.id, npc.hermesProfileId));
  const [profile] = await db.select().from(hermesProfiles).where(eq(hermesProfiles.id, npc.hermesProfileId));
  list = await manager.getTasksByChannel(seed.channelId);
...
for (const terminal of ["complete", "cancelled"]) {
  test(`late agent create/update cannot reopen a manually ${terminal} task`, async () => {
    ...
    const [character] = await db.insert(characters).values({ userId: seed.userId, name: "Tester", appearance: "{}" }).returning();
    const manager = new TaskManager(db, { tasks, npcs, hermesProfiles });
    const backlog = await manager.createBacklogTask(seed.channelId, character.id, "Keep manual result", null);
    ...
    for (const action of ["update", "create", "complete", "cancel"]) {
      const result = await manager.handleTaskAction({
        action, id: backlog.npcTaskId, title: "Late agent title", summary: "Late output", status: "in_progress",
      }, seed.channelId, seed.npcIds[0], character.id);
    ...
    await manager.moveTask(backlog.id, seed.channelId, "in_progress", seed.npcIds[0]);
    const accepted = await manager.handleTaskAction({ action: "update", id: backlog.npcTaskId,
      summary: "New work after explicit resume", status: "in_progress" }, seed.channelId, seed.npcIds[0], character.id);
