...
const tasks = sqliteTable(
  ...
  (table) => [uniqueIndex("idx_tasks_npc_task_id").on(table.npcId, table.npcTaskId)],
);
...
test("handleTaskAction(update) keeps lastReportedAt populated", async () => {
  ...
  sqlite
    .prepare("UPDATE tasks SET last_reported_at = ? WHERE npc_task_id = ?")
    .run("2026-03-31T00:01:00.000Z", "eunyu-1");
...
test("getStaleInProgressTasks falls back to updatedAt when lastReportedAt is null", async () => {
  ...
  sqlite
    .prepare("UPDATE tasks SET updated_at = ?, last_reported_at = NULL WHERE npc_task_id = ?")
