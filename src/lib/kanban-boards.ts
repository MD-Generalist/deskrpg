 ...
 * 캐시를 채운다. 캐시가 `unknown`(도달 실패·타임아웃)이면 정보가 없는 것이므로 다시 찌른다 —
 * 그 값을 한 시간 붙들면 게이트웨이가 살아나도 보드 확보가 한 시간 동안 안 된다.
 */
async function gatePlugin(
  ...
  if (!cached.needsReprobe && cached.status !== "unknown") {
    status = cached.status;
    info = restorePluginInfo(resource.pluginInfoJson);
  } else {
