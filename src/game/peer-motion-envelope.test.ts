...
import { TrafficCoordinator, clearActors, clearTraffic, ACTOR_SEPARATION, segmentDistance } from "./traffic";
...
function simulate(interval: number, latency: number, envelope: boolean, changeMotion = false, doorway = false, stepMs = 10) {
  let a = { id: "a", x: 5, y: 5 }, b = { id: "b", x: 5, y: 9.02 };
  let pa = { ...a, moving: true }, pb = { ...b, moving: true }, receivedAt = 0;
  const queues: { due: number; a: typeof pa; b: typeof pb }[] = [];
  const ca = new TrafficCoordinator(), cb = new TrafficCoordinator();
  let minimum = Infinity, lastSent = -interval, movingA = true, movingB = true;
  const floor = (x: number, y: number) => x >= 0 && x < 15 && y >= 0 && y < 15 && (!doorway || y !== 7 || x === 5 || x === 6);
  for (let now = 0; now < 30000; now += stepMs) {
    if (now - lastSent >= interval) {
      queues.push({ due: now + latency, a: { ...a, moving: movingA }, b: { ...b, moving: movingB } });
      lastSent = now;
    }
    while (queues.length && queues[0].due <= now) {
      const message = queues.shift()!;
      pa = message.a; pb = message.b; receivedAt = now;
    }
    const peer = (sample: typeof pa) => ({
      ...sample, player: true,
      movementUncertainty: envelope ? peerMovementUncertainty({ receivedAt, moving: sample.moving }, now, stepMs) : 0,
    });
    const nextA = ca.step(a.id, a, { x: 5, y: 11 }, 3.75 * stepMs / 1000, now, floor, [a, peer(pb)]);
    // Abrupt stop, perpendicular turn, then reversal happen between reports.
    const stopped = changeMotion && now >= 350 && now < 600;
    const goalB = changeMotion && now >= 600 && now < 900 ? { x: 9, y: b.y }
      : changeMotion && now >= 900 && now < 1200 ? { x: 2, y: b.y } : { x: 5, y: 3 };
    const nextB = stopped ? b : cb.step(b.id, b, goalB, 3.75 * stepMs / 1000, now, floor, [b, peer(pa)]);
    ...
    minimum = Math.min(minimum, segmentDistance(
      { x: a.x - b.x, y: a.y - b.y },
      { x: nextA.x - nextB.x, y: nextA.y - nextB.y }, { x: 0, y: 0 },
    ));
    a = { ...a, ...nextA }; b = { ...b, ...nextB };
...
for (const [interval, latency] of [[10, 0], [66, 0], [66, 30], [66, 66]]) {
  for (const changeMotion of [false, true]) {
    test(`receive-time envelope: ${interval}ms updates/${latency}ms transit, abrupt changes=${changeMotion}`, () => {
      ...
      assert.ok(Math.hypot(result.a.x - 5, result.a.y - 11) < .02, JSON.stringify(result));
      assert.ok(Math.hypot(result.b.x - 5, result.b.y - 3) < .02, JSON.stringify(result));
    });
  }
}
test("peer movement allowance preserves physical seat occupancy, walls and monotone escape", () => {
  const peer = { x: 1, y: 1, movementUncertainty: .6 };
  ...
  assert.equal(clearTraffic({ x: 1.5, y: 1 }, { x: 2, y: 1 }, (x) => x < 2, [peer]), false);
  assert.equal(clearActors({ x: 1.5, y: 1 }, { x: 1.6, y: 1 }, [{ x: 1, y: 1 }, { x: 2, y: 1 }]), false);
...
test("a stopped remote caller remains reachable at the normal 60Hz arrival threshold", () => {
  ...
  for (let now = 0; now < 5000; now += 1000 / 60) {
    ...
    npc = { ...npc, ...traffic.step(npc.id, npc, caller, .05, now, () => true, [npc, {
      ...caller, movementUncertainty: peerMovementUncertainty({ receivedAt: 0, moving: false }, now, 1000 / 60),
    }]) };
  }
