...
export function createOfficeActor(id: string, look: OfficeLook, index: number) {
  ...
  rig.add(torso);
  const isJacket = look.outfit === "suit" || look.outfit === "coat" || look.outfit === "cardigan";
  ...
  if (look.id === "office-jun" || look.id === "office-tae" || look.id === "office-seo") {
    for (const s of [-1, 1])
  ...
  for (const s of [-1, 1]) {
    ...
    round(leg, width, 0.63, 0.22, look.trousers, 0, -0.3, 0, 0.035);
  ...
  if (look.lower === "skirt") {
    skirt = new T.Mesh(
      new T.CylinderGeometry(0.26, 0.34, 0.7, 20, 1, false),
      material(look.trousers),
    );
    skirt.position.set(0, 0.43, 0);
  ...
  return {
    ...
    update(t: number, walking: boolean, phase: ActorPhase, seated: boolean) {
      ...
      if (skirt) {
        ...
        skirt.position.y = sit ? 0.57 : 0.43;
      }
