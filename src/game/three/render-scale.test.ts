...
test('persistent slow tails reduce scale even when median-like FPS is near sixty', () => {
  ...
  assert.deepEqual(adaptRenderScale(first.scale, first.slowSamples, 60, 33.4), { scale: 1.5, slowSamples: 0 });
});
test('one transient slow window does not sacrifice resolution', () => {
  const first = adaptRenderScale(1.5, 0, 40, 40);
  assert.deepEqual(adaptRenderScale(first.scale, first.slowSamples, 60, 18), { scale: 1.5, slowSamples: 0 });
});
test('quality floor is preserved under sustained load', () => {
  assert.deepEqual(adaptRenderScale(.75, 1, 30, 40), { scale: .75, slowSamples: 0 });
});
