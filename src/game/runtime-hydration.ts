...
export type PlayerSpawnState = {
  ...
  restored?: boolean; motion?: PlayerMotionGoal | null;
};
...
export function playerMotionGoal(
  ...
  if (goal) return { targetX: (goal.x + .5) * tileSize, targetY: (goal.y + .5) * tileSize,
    ...(seatId ? { seatId } : {}) };
  if (seatId) {
...
export function copyMotionContinuation(state: MotionContinuation): MotionContinuation {
  return {
    ambientSchedule: { ...state.ambientSchedule,
      ...(state.ambientSchedule.seatTarget ? { seatTarget: { ...state.ambientSchedule.seatTarget } } : {}) },
