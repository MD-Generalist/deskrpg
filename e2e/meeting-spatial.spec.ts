...
test("isolated humans walk, share seats and leave independently on the original canvas", async ({
  ...
  try {
    ...
    const template = templates.find((entry: { name: string }) => /tech|테크/i.test(entry.name));
    expect(template, "seeded official tech template").toBeTruthy();
        ...
        await test.step(kind, async () => {
          const template = templates.find(
            (entry: { name: string; tags: string | null }) =>
              entry.tags === `deskrpg-office-v2:${kind}` ||
